...
{
    std::vector<MyVector::Vector3f> framebuffer(scene.width * scene.height);

    float scale = tan(deg2rad(scene.fov * 0.5));
    float imageAspectRatio = scene.width / (float)scene.height;
    MyVector::Vector3f eye_pos(278, 273, -800);
    int m = 0;
    ...
    auto castRay = [&](int thread_index)
    {
        ...
        for (uint32_t j = height - thread_height; j < height; j++)
        {
            for (uint32_t i = 0; i < scene.width; ++i) {
                ...
                //eye的位置对结果有影响
                MyVector::Vector3f dir = normalize(MyVector::Vector3f(-x, y, 1));
                for (int k = 0; k < spp; k++)
                {
